...
def connect(use_gui=True, shadows=True, color=None, width=None, height=None, mp4=None, fps=120):
    ...
    with HideOutput():
        ...
        if color is not None:
            options.append(
                '--background_color_red={} --background_color_green={} --background_color_blue={}'.format(*color))
...
def create_faces(mesh, scale=1., mass=STATIC_MASS, collision=True, color=GREY, **kwargs):
    ...
    collision_id, visual_id = create_shape(get_faces_geometry(mesh, scale=scale, **kwargs), collision=collision,
                                           color=color)
    body = create_body(collision_id, visual_id, mass=mass)
    # fixed_base = (mass == STATIC_MASS)
...
def clone_body(body, links=None, collision=True, visual=True, client=None):
    ...
    new_body = p.createMultiBody(baseMass=base_dynamics_info.mass,
                                 baseCollisionShapeIndex=clone_collision_shape(body, base_link,
                                                                               client) if collision else NULL_ID,
                                 baseVisualShapeIndex=clone_visual_shape(body, base_link,
                                                                         client) if visual else NULL_ID,
...
def plan_nonholonomic_motion(body, joints, end_conf, obstacles=[], attachments=[],
                             ...
                             max_distance=MAX_DISTANCE, use_aabb=False, cache=True, custom_limits={}, algorithm=None,
                             **kwargs):
...
def control_joint(body, joint, position=None, velocity=0., **kwargs):
    ...
    return p.setJointMotorControl2(bodyIndex=body,  # bodyUniqueId
                                   ...
                                   # Note that the targetVelocity is not the maximum joint velocity
...
def control_joints(body, joints, positions=None, velocities=None, position_gain=None, velocity_scale=None,
                   max_force=None):
    if positions is None:
        positions = get_joint_positions(body, joints)
    if velocities is None:
